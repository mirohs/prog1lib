...
bool check_expect_token(int line, Token actual, Token expected) {
    bool b1 = base_check_expect_i(__FILE__, line, actual.type, expected.type);
    bool b2 = base_check_expect_i(__FILE__, line, actual.start, expected.start);
    bool b3 = base_check_expect_i(__FILE__, line, actual.end, expected.end);
    return b1 && b2 && b3;
}
...
void next_token_test() {
    ...
    check_expect_token(__LINE__, next_token(" 1.34 + 89", 10), make_token(UNKNOWN, 10, 10));
    ...
    check_expect_token(__LINE__, next_token(" ab3", 0), make_token(OPERAND, 3, 4));
}
